          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            {solutions.map((solution, index) => (
              <AnimatedSection key={index} delay={0.1 + index * 0.1}>
                ...
                >
                  {/* Content Container */}
                  <div className="relative bg-white border-l-4 border-brand-orange/30 pl-8 pr-6 py-8 hover:border-brand-orange transition-colors duration-300">
                    {/* Icon */}
                    <div className="mb-6">
                      <div className="w-16 h-16 rounded-lg bg-brand-orange/10 flex items-center justify-center group-hover:bg-brand-orange/20 transition-colors duration-300">
                    ...
                    {/* Badge */}
                    <div className="mb-4">
                      <span className="inline-block px-3 py-1 bg-brand-orange/10 text-brand-orange text-xs font-bold uppercase tracking-wider rounded">
                        {solution.word}
                      </span>
                    </div>

                    {/* Title */}
                    <h3 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4 group-hover:text-brand-orange transition-colors duration-300">
                      {solution.title}
                    </h3>
                    ...
                    <ul className="space-y-3 mb-8">
                      {solution.features.map((feature, idx) => (
                        <li key={idx} className="flex items-center text-sm text-gray-700">
                          <div className="w-5 h-5 rounded-full bg-brand-orange/10 flex items-center justify-center mr-3 flex-shrink-0">
                    ...
                    <motion.a
                      href={solution.link}
                      className="inline-flex items-center text-brand-orange font-semibold group/link"
                      whileHover={{ x: 5 }}
                      transition={{ type: 'spring', stiffness: 400 }}
                    >
                      <span>Learn more</span>
                      <svg className="ml-2 w-5 h-5 group-hover/link:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
